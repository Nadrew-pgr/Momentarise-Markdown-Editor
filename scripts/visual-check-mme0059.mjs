...
const visualDir = "docs/internal/visual-checks/MME-0059";
const port = 18400 + Math.floor(Math.random() * 500);
const userDataDir = `/tmp/mme-visual-0059-${Date.now()}`;
...
async function main() {
  ...
  try {
    ...
    await evaluate(
      cdp,
      `window.__MME_DEMO_VISUAL_CHECK__.loadWritableMarkdownFileForTest("multiline-footnotes.md", ${JSON.stringify(source)})`
    );
    ...
    assert(
      await evaluate(
        cdp,
        `document.querySelector('[data-mme-footnote-definition="true"]')?.textContent.includes("Second definition line")`
      ),
      "Continuation definition must render all logical lines."
    );
    await screenshot(cdp, "footnote-multiline-rich-desktop.png");

    await evaluate(cdp, `window.__MME_DEMO_VISUAL_CHECK__.selectRichTextForTest("Second definition line")`);
    await evaluate(cdp, `window.__MME_DEMO_VISUAL_CHECK__.typeRichTextForTest("Edited second definition line")`);
    await waitFor(
      cdp,
      `window.__MME_DEMO_VISUAL_CHECK__.getMarkdown().includes("    Edited second definition line has")`,
      "continuation definition edit"
    );
    assert(
      await evaluate(
        cdp,
        `getComputedStyle(document.querySelector('[data-mme-footnote-marker="true"]'), '::before').content.includes('[^long]:')`
      ),
      "Definition marker must survive a continuation-line edit."
    );
    const editedMarkdown = await evaluate(cdp, `window.__MME_DEMO_VISUAL_CHECK__.getMarkdown()`);
    assert(editedMarkdown.includes("[^long]: First **definition** line stays.\n    Edited second definition line has `inline code`.\n    Third definition line has a [relative link](./target.md)."), "Continuation indentation and inline Markdown must survive the edit.");
    assert(editedMarkdown.includes("[^multi]: First paragraph stays source-only.\n\n    Second paragraph stays source-only."), "Multi-paragraph definition must remain exact.");
    assert(editedMarkdown.includes("> [^nested]: Nested definition stays source-only."), "Nested definition must remain exact.");
    assert(editedMarkdown.includes('Neighbor <x-unknown keep="exact">syntax</x-unknown> stays byte-exact.'), "Unknown neighboring syntax must remain exact.");
    ...
    await screenshot(cdp, "footnote-multiline-edited-desktop.png");
    ...
    await screenshot(cdp, "footnote-multiline-constrained.png");
    ...
    await screenshot(cdp, "footnote-multiline-source-desktop.png");
    cdp.close();
    console.log(`MME-0059 runtime artifacts saved to ${visualDir}`);
  } catch (error) {
    if (stderr.trim()) {
