...
const visualDir = "docs/internal/visual-checks/MME-0008";
const port = 10500 + Math.floor(Math.random() * 500);
const userDataDir = `/tmp/mme-visual-0008-${Date.now()}`;
...
async function getSaveSnapshot(cdp) {
  return evaluate(
    cdp,
    `(() => {
      ...
      return {
        ...
        externalHashLabel: document.querySelector('[data-testid="save-engine-external-hash"]').textContent,
        lastAction: document.querySelector('[data-testid="save-engine-last-action"]').textContent,
        lastSavedHash: state.lastSavedHash ?? null,
...
async function waitForSaveState(cdp, predicate, label) {
  const start = Date.now();
  ...
  while (Date.now() - start < 6000) {
    ...
    await wait(100);
    snapshot = await getSaveSnapshot(cdp);
  }
...
async function main() {
  ...
  try {
    ...
    const dirty = await waitForSaveState(
      cdp,
    ...
    await evaluate(cdp, "window.__MME_DEMO_VISUAL_CHECK__.setCursorToEnd()");
    await cdp.send("Input.insertText", {
      text: "\nMME-0008 shortcut edit."
    });
    await waitForSaveState(cdp, (snapshot) => snapshot.status === "dirty", "dirty before shortcut");
    await keyChord(cdp, "s");
    const shortcut = await waitForSaveState(
      cdp,
      (snapshot) =>
        snapshot.status === "saved" &&
        snapshot.target === "memory-only" &&
    ...
    await evaluate(cdp, "window.__MME_DEMO_VISUAL_CHECK__.setCursorToEnd()");
    await cdp.send("Input.insertText", {
      text: "\nMME-0008 conflict candidate."
    });
    await waitForSaveState(cdp, (snapshot) => snapshot.status === "dirty", "dirty before conflict");
    await evaluate(cdp, "window.__MME_DEMO_VISUAL_CHECK__.simulateExternalConflict()");
    await keyChord(cdp, "s");
    const conflict = await waitForSaveState(
      cdp,
      (snapshot) => snapshot.status === "conflict" && snapshot.target === "conflict",
      "conflict state"
    );
    assertIncludes(conflict.saveLabel, "conflict", "conflict save label");
    assertIncludes(conflict.targetLabel, "not overwritten", "conflict target label");
    assertIncludes(conflict.lastAction, "blocked overwrite", "visible conflict action");
    if (conflict.externalHashLabel === "none") {
      throw new Error("Conflict screenshot must expose an external hash label.");
    }
    await screenshot(cdp, "save-engine-conflict.png");

    cdp.close();
    console.log(`MME-0008 visual artifacts saved to ${visualDir}`);
  } catch (error) {
    if (stderr.trim()) {
