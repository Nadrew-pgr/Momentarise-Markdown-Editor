...
const visualDir = "docs/internal/visual-checks/MME-0073";
const port = 22000 + Math.floor(Math.random() * 500);
const userDataDir = `/tmp/mme-visual-0073-${Date.now()}`;
const fixturePath = "fixtures/037-table-column-operations/input.md";
...
async function main() {
  ...
  const insertedSource = source.replace(
    "       | Key | State |\n       | :--- | :---: |\n       | ordered one | ready |\n       | ordered two | exact |",
    "       | Key |  | State |\n       | :--- | --- | :---: |\n       | ordered one |  | ready |\n       | ordered two |  | exact |"
  );
  const finalSource = insertedSource.replace(
    "       | Key |  | State |\n       | :--- | --- | :---: |\n       | ordered one |  | ready |\n       | ordered two |  | exact |",
    "       |  | State |\n       | --- | :---: |\n       |  | ready |\n       |  | exact |"
  );
  const chrome = spawn(
  ...
  try {
    ...
    await evaluate(
      cdp,
      `window.__MME_DEMO_VISUAL_CHECK__.loadWritableMarkdownFileForTest("table-column-operations.md", ${JSON.stringify(source)})`
    );
    ...
    assert(
      ...
      mounted.tables === 5 &&
      mounted.rootRows === 3 &&
      mounted.directRows === 3 &&
      mounted.orderedRows === 3 &&
      mounted.taskRows === 3 &&
    ...
    const unavailableActions = await evaluate(cdp, `(() => {
      const ids = ['mme:tableColumnBefore', 'mme:tableColumnAfter', 'mme:tableColumnDelete'];
      return ids.map((id) => {
    ...
    assert(
      ...
      unavailableActions.map((action) => action.label).join("|") === "Insert column before|Insert column after|Delete column",
      `Column commands must be labelled and unavailable outside tables: ${JSON.stringify(unavailableActions)}`
    );
    await screenshot(cdp, "table-column-commands-unavailable.png");
    await evaluate(cdp, `document.querySelector('[data-testid="toolbar-more-button"]')?.click()`);

    await evaluate(cdp, `window.__MME_DEMO_VISUAL_CHECK__.selectRichTableCellForTest(1, 0, 4)`);
    await evaluate(cdp, `document.querySelectorAll('[data-testid="rich-editor-host"] .ProseMirror table')[4]?.scrollIntoView({ block: "center" })`);
    await wait(150);
    assert(await evaluate(cdp, `(() => {
      const table = document.querySelectorAll('[data-testid="rich-editor-host"] .ProseMirror table')[4];
      const rect = table?.getBoundingClientRect();
      return Boolean(rect && rect.bottom > 100 && rect.top < window.innerHeight);
    })()`), "Protected one-column table must be visible in its visual proof.");
    await evaluate(cdp, `document.querySelector('[data-testid="toolbar-more-button"]')?.click()`);
    ...
    assert(
      protectedActions[0] === false && protectedActions[1] === false && protectedActions[2] === true,
      `Final-column deletion must be disabled while insertion remains available: ${JSON.stringify(protectedActions)}`
    );
    await screenshot(cdp, "table-column-final-protected.png");
    await evaluate(cdp, `document.querySelector('[data-testid="toolbar-more-button"]')?.click()`);

    await evaluate(cdp, `window.__MME_DEMO_VISUAL_CHECK__.selectRichTableCellForTest(1, 0, 2)`);
    await evaluate(cdp, `document.querySelectorAll('[data-testid="rich-editor-host"] .ProseMirror table')[2]?.scrollIntoView({ block: "center" })`);
    await wait(150);
    assert(await evaluate(cdp, `(() => {
      const table = document.querySelectorAll('[data-testid="rich-editor-host"] .ProseMirror table')[2];
      const rect = table?.getBoundingClientRect();
      return Boolean(rect && rect.bottom > 100 && rect.top < window.innerHeight);
    })()`), "Enabled nested table must be visible in its visual proof.");
    await evaluate(cdp, `document.querySelector('[data-testid="toolbar-more-button"]')?.click()`);
    const enabledActions = await evaluate(cdp, `(() => {
      const ids = ['mme:tableColumnBefore', 'mme:tableColumnAfter', 'mme:tableColumnDelete'];
      return ids.map((id) => document.querySelector('[data-toolbar-command-id="' + id + '"]')?.disabled ?? null);
    })()`);
    assert(enabledActions.every((disabled) => disabled === false), `Table-column commands must be enabled: ${JSON.stringify(enabledActions)}`);
    await screenshot(cdp, "table-column-commands-enabled.png");
    await evaluate(cdp, `document.querySelector('[data-testid="toolbar-more-button"]')?.click()`);

    assert(await evaluate(cdp, `window.__MME_DEMO_VISUAL_CHECK__.runRichCommand("tableColumnAfter")`), "Nested insert-column command was not handled.");
    await waitFor(cdp, `window.__MME_DEMO_VISUAL_CHECK__.getMarkdown() === ${JSON.stringify(insertedSource)}`, "nested column insertion");
    await waitFor(cdp, `Array.from(document.querySelectorAll('[data-mme-footnote-identifier="ordered"] table tr')).every((row) => row.children.length === 3)`, "inserted nested column DOM");
    await screenshot(cdp, "table-column-inserted.png");

    await pressKey(cdp, "z", { metaKey: true });
    await waitFor(cdp, `window.__MME_DEMO_VISUAL_CHECK__.getMarkdown() === ${JSON.stringify(source)}`, "column insertion undo");
    await pressKey(cdp, "z", { metaKey: true, shiftKey: true });
    await waitFor(cdp, `window.__MME_DEMO_VISUAL_CHECK__.getMarkdown() === ${JSON.stringify(insertedSource)}`, "column insertion redo");
    await evaluate(cdp, `window.__MME_DEMO_VISUAL_CHECK__.selectRichTableCellForTest(1, 0, 2)`);
    assert(await evaluate(cdp, `window.__MME_DEMO_VISUAL_CHECK__.runRichCommand("tableColumnDelete")`), "Nested delete-column command was not handled.");
    await waitFor(cdp, `window.__MME_DEMO_VISUAL_CHECK__.getMarkdown() === ${JSON.stringify(finalSource)}`, "nested column deletion");
    await screenshot(cdp, "table-column-deleted.png");
    ...
    await screenshot(cdp, "table-column-saved-desktop.png");
    ...
    await wait(150);
    await screenshot(cdp, "table-column-unsupported-desktop.png");
    ...
    await screenshot(cdp, "table-column-constrained.png");

    await evaluate(cdp, `window.__MME_DEMO_VISUAL_CHECK__.selectRichTableCellForTest(1, 0, 5)`);
    ...
    await screenshot(cdp, "table-column-wide-constrained.png");
    ...
    await screenshot(cdp, "table-column-source-desktop.png");
    cdp.close();
    console.log(`MME-0073 runtime artifacts saved to ${visualDir}`);
  } catch (error) {
    if (stderr.trim()) {
