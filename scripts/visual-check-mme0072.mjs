...
const visualDir = "docs/internal/visual-checks/MME-0072";
const port = 21500 + Math.floor(Math.random() * 500);
const userDataDir = `/tmp/mme-visual-0072-${Date.now()}`;
const fixturePath = "fixtures/036-table-row-operations/input.md";
...
async function main() {
  ...
  const finalSource = insertedSource.replace("       | ordered one | ready |\n", "");
  const chrome = spawn(
    chromePath,
    [
      "--headless=new",
  ...
  try {
    ...
    await evaluate(
      cdp,
      `window.__MME_DEMO_VISUAL_CHECK__.loadWritableMarkdownFileForTest("table-row-operations.md", ${JSON.stringify(source)})`
    );
    ...
    const unavailableActions = await evaluate(cdp, `(() => {
      const ids = ['mme:tableRowBefore', 'mme:tableRowAfter', 'mme:tableRowDelete'];
      return ids.map((id) => {
    ...
    assert(
      ...
      unavailableActions.map((action) => action.label).join("|") === "Insert row before|Insert row after|Delete row",
      `Row commands must be labelled and unavailable outside body cells: ${JSON.stringify(unavailableActions)}`
    );
    await screenshot(cdp, "table-row-commands-unavailable.png");
    await evaluate(cdp, `document.querySelector('[data-testid="toolbar-more-button"]')?.click()`);
    ...
    assert(enabledActions.every((disabled) => disabled === false), `Body-row commands must be enabled: ${JSON.stringify(enabledActions)}`);
    await screenshot(cdp, "table-row-commands-enabled.png");
    await evaluate(cdp, `document.querySelector('[data-testid="toolbar-more-button"]')?.click()`);

    assert(await evaluate(cdp, `window.__MME_DEMO_VISUAL_CHECK__.runRichCommand("tableRowAfter")`), "Nested insert-row command was not handled.");
    await waitFor(cdp, `window.__MME_DEMO_VISUAL_CHECK__.getMarkdown() === ${JSON.stringify(insertedSource)}`, "nested row insertion");
    await waitFor(cdp, `document.querySelector('[data-mme-footnote-identifier="ordered"] table')?.querySelectorAll('tr').length === 4`, "inserted nested row DOM");
    await screenshot(cdp, "table-row-inserted.png");

    await pressKey(cdp, "z", { metaKey: true });
    await waitFor(cdp, `window.__MME_DEMO_VISUAL_CHECK__.getMarkdown() === ${JSON.stringify(source)}`, "row insertion undo");
    await pressKey(cdp, "z", { metaKey: true, shiftKey: true });
    await waitFor(cdp, `window.__MME_DEMO_VISUAL_CHECK__.getMarkdown() === ${JSON.stringify(insertedSource)}`, "row insertion redo");
    await evaluate(cdp, `window.__MME_DEMO_VISUAL_CHECK__.selectRichTableCellForTest(1, 0, 2)`);
    assert(await evaluate(cdp, `window.__MME_DEMO_VISUAL_CHECK__.runRichCommand("tableRowDelete")`), "Nested delete-row command was not handled.");
    await waitFor(cdp, `window.__MME_DEMO_VISUAL_CHECK__.getMarkdown() === ${JSON.stringify(finalSource)}`, "nested row deletion");
    await screenshot(cdp, "table-row-deleted.png");
    ...
    await screenshot(cdp, "table-row-saved-desktop.png");
    ...
    await wait(150);
    await screenshot(cdp, "table-row-unsupported-desktop.png");
    ...
    await screenshot(cdp, "table-row-constrained.png");
    ...
    await screenshot(cdp, "table-row-wide-constrained.png");
    ...
    await screenshot(cdp, "table-row-source-desktop.png");
    cdp.close();
    console.log(`MME-0072 runtime artifacts saved to ${visualDir}`);
  } catch (error) {
    if (stderr.trim()) {
