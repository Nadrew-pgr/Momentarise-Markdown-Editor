...
/*
 * MME-0116: this used to read each PNG off disk and reject one under 1000 bytes.
 * Screenshots are gate output now — gitignored, regenerated by every `npm run visual`,
 * uploaded by CI — so reading them here would make `npm test` depend on whether the
 * browser suite had been run on this machine. "The gate produced its proof" is the
 * runner's job and it checks the stronger property: written *during this run*, not
 * merely present. The naming contract above still fails if the gate stops writing one.
 */
for (const artifact of ["markdown-read-view.png", "markdown-read-sanitized-html.png"]) {
  if (!visualScript.includes(artifact)) {
    throw new Error(`MME-0032 visual script no longer writes ${artifact}.`);
  }
  if (!readFileSync("docs/internal/visual-checks/MME-0032/README.md", "utf8").includes(artifact)) {
    throw new Error(`MME-0032 visual README no longer documents ${artifact}.`);
  }
}
