...
- Mount one paragraph-only blockquote child, one safe indented-code child, one safe table child, and one safe callout child semantically; keep multiple containers, nested quotes, raw HTML, nested-container definitions, duplicates, stale source, and unmappable layouts source-only.
