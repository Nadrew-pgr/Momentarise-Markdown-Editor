...
- Mount one paragraph-only blockquote child, one safe indented-code child, one safe table child, one safe callout child, and one inert raw-HTML source child semantically; keep multiple containers, nested quotes, nested-container definitions, duplicates, stale source, and unmappable layouts source-only.
