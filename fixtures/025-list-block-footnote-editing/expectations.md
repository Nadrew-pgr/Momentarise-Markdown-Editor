...
- Mount the unique top-level `steps` and `ordered` definitions as semantic editable Rich definitions.
- Represent safe bullet and ordered lists with package-owned ProseMirror list nodes and preserve ordered start value `3`.
...
- Keep nested lists, task lists, loose or multi-paragraph list items, quotes, raw HTML, nested-container definitions, duplicates, malformed definitions, stale source, and unmappable ranges source-only.
