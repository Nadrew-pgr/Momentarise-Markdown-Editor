...
Unique top-level GFM definitions with representable inline content mount as editable semantic blocks. This includes single-line definitions, single paragraphs continued on consistently indented source lines, multiple plain paragraphs separated by valid indented blank-line structure, and standard bullet or ordered lists whose items each contain one representable paragraph plus at most one recursively safe nested standard list. Unchanged child-block source stays byte-exact when a sibling block is edited. A changed list block is reconstructed deterministically inside its bounded definition range while retaining representable ordered-list start values, hierarchy, and footnote container indentation. References remain semantic inline atoms and retain their original Markdown spelling. New and renamed references and definitions use exact source mapping, preserve unrelated Markdown bytes and line endings, and refuse unsupported, ambiguous, colliding, partially mapped, or stale operations. Task lists, loose or multi-paragraph list items, multiple nested child blocks, other nested blocks, nested-container definitions, duplicates, malformed definitions, unsafe content, inconsistent indentation, and otherwise non-representable definitions remain visible source-only fallbacks.
