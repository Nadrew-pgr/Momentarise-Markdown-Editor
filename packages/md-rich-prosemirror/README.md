...
- `runRichTableMatrixPaste` pastes a bounded rectangular TSV matrix by default, or explicit quoted CSV with `format: "csv"`, into a selected or explicit supported table cell. Both paths expand right/down in one transaction while keeping spreadsheet values literal.
...
Native Rich paste accepts explicit `text/tab-separated-values`, explicit `text/csv`, or `text/plain` containing tabs while selection is inside a supported table and no file payload is present. TSV wins when clipboard formats overlap; ordinary comma-containing text is never inferred as CSV. CSV parsing uses the MIT-licensed `csv-parse` browser sync entrypoint and supports quoted commas, doubled quotes, empty cells, BOM, LF, and CRLF. Multiline cells are rejected because GFM pipe tables cannot persist literal line breaks losslessly. Input is rectangular, control-safe, capped at 1,000 rows, 256 columns, 10,000 cells, and 1,000,000 UTF-16 code units. Accepted paste prevents default, expands the table, selects the final cell, and remains one undoable transaction. Rejected matrices plus normal text, HTML, images, source-only tables, and outside-table paste remain on existing host/editor paths. Pasted values serialize as literal Markdown-safe table text; authored semantic links and marks remain semantic.
