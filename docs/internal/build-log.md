...
## MME-0015 follow-up — HTML preview responsive frame

- Timestamp: 2026-06-01T17:06:18Z
- Summary: Addressed human feedback that real HTML artifacts could render too wide for the demo preview surface, causing clipped content and scroll confusion. The first attempt used preview-only zoom, but that was rejected because the goal is a real responsive iframe viewport like browser/device tools. The demo now keeps app-level horizontal overflow contained, makes the narrow viewport layout more resilient, and provides Fit/Mobile/Tablet/Desktop preview frame sizes. The source HTML remains unchanged and artifact content is not zoomed.
- Files changed:
  - `apps/md-demo/src/main.ts`
  - `apps/md-demo/src/styles.css`
  - `scripts/visual-check-mme0015.mjs`
  - `tests/demo-html-preview-baseline.test.mjs`
  - `docs/internal/visual-checks/MME-0015/README.md`
  - `docs/internal/visual-checks/MME-0015/html-preview-responsive-frame.png`
  - `docs/internal/ISSUES.md`
  - `docs/internal/PRD.md`
- Behavior proven:
  - The page does not gain horizontal scroll when previewing a wide, non-responsive HTML artifact.
  - The HTML preview host and scroll stage stay within a narrow viewport.
  - Wide non-responsive artifact overflow remains inside the preview stage/iframe, not on the app page.
  - Responsive HTML content sees the selected iframe width as its viewport.
  - Artifact content is not zoomed to fake responsiveness.
  - The sandbox still disables scripts and source/preview flow still uses the real `.html` file input path.
- Test-first evidence:
  - Updated `tests/demo-html-preview-baseline.test.mjs` and `scripts/visual-check-mme0015.mjs` first.
  - Confirmed `npm run test:demo-html-preview` failed before implementation on missing responsive-frame UI evidence.
- Tests/checks run:
  - `npm run test:demo-html-preview`
  - `npm run visual:mme-0015`
  - `npm run test:html-preview`
...
- Visual artifacts:
  ...
  - `docs/internal/visual-checks/MME-0015/html-preview-responsive-frame.png`
- Visual impact:
  - HTML Preview: the preview now behaves like a responsive frame. `Fit` uses the available width; `Mobile`, `Tablet`, and `Desktop` set explicit iframe widths so responsive HTML can reflow naturally.
  - General UI: the demo is more stable in narrower viewports; top actions wrap, the workspace collapses to one column below the responsive breakpoint, and the technical inspector moves below the editor instead of squeezing the preview. Non-responsive HTML may scroll within the preview stage, not the app page.
  - Editing surface: no intended Markdown source/rich editing behavior change.
- Product notes:
  - Obsidian-like inline/block HTML rendering inside Markdown is not part of this MME-0015 artifact reader. It is now explicitly tracked for MME-0020/live-preview hardening, with policy-aware rendering and raw `.md` preservation.
  - The Source/Rich/Preview control is still demo-style. MME-0020 now explicitly requires a document-kind-aware mode control so Markdown and HTML preview choices do not feel duplicated or confusing.
- Reviewer/subagent used and result:
  - Not used for this follow-up; this was a focused human-reported visual regression with automated visual proof added.
- Suggested commit message:
  - `fix: use responsive HTML preview frame`
