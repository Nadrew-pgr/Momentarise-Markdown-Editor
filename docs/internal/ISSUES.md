...
- CLI and AX open agent-first (D18): "l'agent c'est ce qui est le plus utilisé, et pour les non dev surtout". `md-cli.md` and `agentic-experience.md` lead with the agent manipulating MME documents — often on behalf of a non-developer — and address developers second. This reverses today's ordering, which assumes a developer at a terminal.
- The roadmap shows the real trajectory (D19), in Andrew's stated order: docs-site-on-MME, then the Payload path, then SEO/AEO pages, then the back-office — ahead of mobile and VS Code. "on montre, pour mme, c'est plus propre pour moi aussi". This replaces the current ordering, which predates every decision in this record.
- Localization gets a public page (D21): document `MmeStrings` and `defaultMmeStrings`, ship a French reference dictionary, and state visibly — in the FAQ or CONTRIBUTING — that the documentation itself is English-only **as a recorded decision, not an omission**. A reader who finds no French docs must be able to see that it was decided rather than forgotten.
- AX stays one page (D20): no adoption-versus-runtime structural split. The two uses (setting MME up in an app; agents writing `.md` files) may be clarified in passing, without doctrine.
- Performance framing is confirmed as-is (D17): guardrails, never a marketing axis. No performance claim ships that has not been measured, and the residual risk — very large documents need virtualization work — stays documented rather than smoothed over.
