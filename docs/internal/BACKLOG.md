...
- Shared `todo_item` DOM semantics for standalone, bullet/ordered, nested, and footnote tasks are promoted as `MME-0077`; keep native button focus/state behavior while ensuring valid list and assistive-technology structure.
...
- MME-0077 todo semantics review: review unordered marker suppression, ordered numbering, checkbox alignment/density, checked styling, pointer/keyboard focus treatment, nested hierarchy, task creation/nesting/undo flow, Source visibility, and constrained layout.
